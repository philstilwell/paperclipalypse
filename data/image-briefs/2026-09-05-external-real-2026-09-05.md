...
Run: 2026-09-05-external-real-2026-09-05
Winner: Gemini Flash
Winning joke: Down on the Tarmac
...
Optional small header text above the panel: Winner: Gemini Flash - Down on the Tarmac
Exact joke text to render in the right panel:
I grew up on a dairy farm, so whenever a flight attendant starts demonstrating how the oxygen mask drops from the ceiling, my muscle memory kicks in and I immediately reach up to try and milk it.
...
Round seed terms for exclusion reference only: Legal, Flight Attendant, Farm, Experiencing discrimination, Persistent, Disrespectful. These are not scene ingredients unless the winning joke explicitly uses them.
...
```sh
node scripts/prepare-feature-image-prompt.mjs --brief data/image-briefs/2026-09-05-external-real-2026-09-05.md
```

Read the generated `tmp/gemini-feature-images/2026-09-05-external-real-2026-09-05.prompt.txt` file directly when filling Gemini's composer.
...
```sh
node scripts/qa-feature-image.mjs --image tmp/gemini-feature-images/2026-09-05-external-real-2026-09-05.jpg --approved
```

After visual approval, render the episode with the approved preview image:

```sh
node scripts/run-tournament.mjs --episode-file data/inbox/external-real-2026-09-05.json --feature-image tmp/gemini-feature-images/2026-09-05-external-real-2026-09-05.jpg --feature-image-qa-approved
```
