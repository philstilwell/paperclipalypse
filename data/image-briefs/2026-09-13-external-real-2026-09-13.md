...
Run: 2026-09-13-external-real-2026-09-13
Winner: Claude
Winning joke: Ring Shopping
...
Optional small header text above the panel: Winner: Claude - Ring Shopping
Exact joke text to render in the right panel:
I'm indecisive, so proposing was a nightmare. I stood at the jewelry counter for two hours. The guy finally said, 'Sir, this is a ring, not a life philosophy.' I said, 'What if she doesn't like princess cut?' He said, 'Then she doesn't like the shape of a rock, get help.' I bought a gift card. She said yes to the gift card.
...
Round seed terms for exclusion reference only: Gothic, Teacher, Jewelry Store, Having a one-night stand, Witty, Indecisive. These are not scene ingredients unless the winning joke explicitly uses them.
...
```sh
node scripts/prepare-feature-image-prompt.mjs --brief data/image-briefs/2026-09-13-external-real-2026-09-13.md
```

Read the generated `tmp/gemini-feature-images/2026-09-13-external-real-2026-09-13.prompt.txt` file directly when filling Gemini's composer.
...
```sh
node scripts/qa-feature-image.mjs --image tmp/gemini-feature-images/2026-09-13-external-real-2026-09-13.jpg --approved
```

After visual approval, render the episode with the approved preview image:

```sh
node scripts/run-tournament.mjs --episode-file data/inbox/external-real-2026-09-13.json --feature-image tmp/gemini-feature-images/2026-09-13-external-real-2026-09-13.jpg --feature-image-qa-approved
```
