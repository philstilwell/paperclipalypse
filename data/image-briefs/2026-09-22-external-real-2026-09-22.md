...
Run: 2026-09-22-external-real-2026-09-22
Winner: Claude
Winning joke: Home Sweet Compromise
...
Optional small header text above the panel: Winner: Claude - Home Sweet Compromise
Exact joke text to render in the right panel:
I hired a real estate agent to help me buy my first house. Told her my budget. She looked at me the way you look at a raccoon that's asked for a mortgage. Showed me one place with a real kitchen, real walls, real plumbing — way over budget. Then she showed me one 'within my range.' It was a campsite. She called it 'rustic charm, motivated seller.' I said 'there's a tent in the listing photo.' She said 'that conveys.'
...
Round seed terms for exclusion reference only: Legal, Real Estate Agent, Campsite, A challenge beyond skill, Objective, Inhibited. These are not scene ingredients unless the winning joke explicitly uses them.
...
```sh
node scripts/prepare-feature-image-prompt.mjs --brief data/image-briefs/2026-09-22-external-real-2026-09-22.md
```

Read the generated `tmp/gemini-feature-images/2026-09-22-external-real-2026-09-22.prompt.txt` file directly when filling Gemini's composer.
...
```sh
node scripts/qa-feature-image.mjs --image tmp/gemini-feature-images/2026-09-22-external-real-2026-09-22.jpg --approved
```

After visual approval, render the episode with the approved preview image:

```sh
node scripts/run-tournament.mjs --episode-file data/inbox/external-real-2026-09-22.json --feature-image tmp/gemini-feature-images/2026-09-22-external-real-2026-09-22.jpg --feature-image-qa-approved
```
