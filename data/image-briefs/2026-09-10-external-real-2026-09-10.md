...
Run: 2026-09-10-external-real-2026-09-10
Winner: Claude
Winning joke: Home Invasion
...
Optional small header text above the panel: Winner: Claude - Home Invasion
Exact joke text to render in the right panel:
My dorm room got broken into freshman year. Nothing stolen. Instead, my laundry was folded, my desk was alphabetized, and there was a Tupperware of lasagna in a mini-fridge that had never seen food before. I called campus security panicking. They ran it and said, 'Ma'am, that's not a break-in, that's just your mother with a spare key.'
...
Round seed terms for exclusion reference only: Young Adult, Diplomat, Dorm Room, A break-in, Confident, Workaholic. These are not scene ingredients unless the winning joke explicitly uses them.
...
```sh
node scripts/prepare-feature-image-prompt.mjs --brief data/image-briefs/2026-09-10-external-real-2026-09-10.md
```

Read the generated `tmp/gemini-feature-images/2026-09-10-external-real-2026-09-10.prompt.txt` file directly when filling Gemini's composer.
...
```sh
node scripts/qa-feature-image.mjs --image tmp/gemini-feature-images/2026-09-10-external-real-2026-09-10.jpg --approved
```

After visual approval, render the episode with the approved preview image:

```sh
node scripts/run-tournament.mjs --episode-file data/inbox/external-real-2026-09-10.json --feature-image tmp/gemini-feature-images/2026-09-10-external-real-2026-09-10.jpg --feature-image-qa-approved
```
