...
Run: 2026-09-11-external-real-2026-09-11
Winner: Gemini Flash
Winning joke: The Deep End
...
Optional small header text above the panel: Winner: Gemini Flash - The Deep End
Exact joke text to render in the right panel:
I was relaxing at a luxury resort pool in a red polo, minding my own business, when a woman panicked and screamed, 'Lifeguard, do something, he's drowning!' I didn't want to cause an awkward scene, so I jumped in and drowned with him.
...
Round seed terms for exclusion reference only: Generation Ship, Party Planner, Pool, Being mistaken for someone else, Professional, Inflexible. These are not scene ingredients unless the winning joke explicitly uses them.
...
```sh
node scripts/prepare-feature-image-prompt.mjs --brief data/image-briefs/2026-09-11-external-real-2026-09-11.md
```

Read the generated `tmp/gemini-feature-images/2026-09-11-external-real-2026-09-11.prompt.txt` file directly when filling Gemini's composer.
...
```sh
node scripts/qa-feature-image.mjs --image tmp/gemini-feature-images/2026-09-11-external-real-2026-09-11.jpg --approved
```

After visual approval, render the episode with the approved preview image:

```sh
node scripts/run-tournament.mjs --episode-file data/inbox/external-real-2026-09-11.json --feature-image tmp/gemini-feature-images/2026-09-11-external-real-2026-09-11.jpg --feature-image-qa-approved
```
