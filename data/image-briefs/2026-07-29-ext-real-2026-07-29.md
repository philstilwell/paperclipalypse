...
Run: 2026-07-29-ext-real-2026-07-29
Winner: OpenAI GPT-5.4 Mini
Winning joke: Off the Record
...
Optional small header text above the panel: Winner: OpenAI GPT-5.4 Mini - Off the Record
Exact joke text to render in the right panel:
I married a journalist, so every argument has ground rules. I say, “Off the record,” and she says the rules changed—that only works before I say something stupid. Apparently “off the record” isn’t a time machine; it’s a tiny umbrella you open after the piano lands.
...
Round seed terms for exclusion reference only: Journalist, Emergency Room Physician, Living Room, Rules changing, Empathetic, Cowardly. These are not scene ingredients unless the winning joke explicitly uses them.
...
```sh
node scripts/qa-feature-image.mjs --image tmp/gemini-feature-images/2026-07-29-ext-real-2026-07-29.jpg --approved
```

After visual approval, render the episode with the approved preview image:

```sh
node scripts/run-tournament.mjs --episode-file data/inbox/ext-real-2026-07-29.json --feature-image tmp/gemini-feature-images/2026-07-29-ext-real-2026-07-29.jpg --feature-image-qa-approved
```
