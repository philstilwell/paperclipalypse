...
Run: 2026-07-11-ext-real-2026-07-11
Winner: Gemini Flash
Winning joke: Goth Retriever
...
Optional small header text above the panel: Winner: Gemini Flash - Goth Retriever
Exact joke text to render in the right panel:
My high school goth phase was a total disaster. You can't successfully project a black and dark aura when you have the completely uninhibited personality of a golden retriever. I’d be stomping around the mall in a heavy trench coat and combat boots, just aggressively waving at people like, “Hey! Life is a bottomless abyss, but these Auntie Anne's pretzel bites are amazing!”
...
Round seed terms for exclusion reference only: Black and Dark, Inventor, Used Car Dealership, An unexpected change of plans, Uninhibited, Resentful. These are not scene ingredients unless the winning joke explicitly uses them.
...
```sh
node scripts/qa-feature-image.mjs --image tmp/gemini-feature-images/2026-07-11-ext-real-2026-07-11.jpg --approved
```

After visual approval, render the episode with the approved preview image:

```sh
node scripts/run-tournament.mjs --episode-file data/inbox/ext-real-2026-07-11.json --feature-image tmp/gemini-feature-images/2026-07-11-ext-real-2026-07-11.jpg --feature-image-qa-approved
```
