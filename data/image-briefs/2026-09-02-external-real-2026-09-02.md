...
Run: 2026-09-02-external-real-2026-09-02
Winner: Gemini Flash
Winning joke: Country Drive
...
Optional small header text above the panel: Winner: Gemini Flash - Country Drive
Exact joke text to render in the right panel:
City people romanticize driving down a dark country road until their headlights hit an antique rocking chair sitting in an empty field. You don’t even need to believe in ghosts. Every buried childhood fear comes right back. My GPS said, 'In half a mile, turn around,' and my engine wasn’t even off yet. That’s not navigation; that’s the car agreeing we’re about to be murdered.
...
Round seed terms for exclusion reference only: Manga, Jewelry Designer, Country Road, A fear or phobia rearing its head, Centred, Suspicious. These are not scene ingredients unless the winning joke explicitly uses them.
...
```sh
node scripts/prepare-feature-image-prompt.mjs --brief data/image-briefs/2026-09-02-external-real-2026-09-02.md
```

Read the generated `tmp/gemini-feature-images/2026-09-02-external-real-2026-09-02.prompt.txt` file directly when filling Gemini's composer.
...
```sh
node scripts/qa-feature-image.mjs --image tmp/gemini-feature-images/2026-09-02-external-real-2026-09-02.jpg --approved
```

After visual approval, render the episode with the approved preview image:

```sh
node scripts/run-tournament.mjs --episode-file data/inbox/external-real-2026-09-02.json --feature-image tmp/gemini-feature-images/2026-09-02-external-real-2026-09-02.jpg --feature-image-qa-approved
```
