...
Run: 2026-09-15-external-real-2026-09-15
Winner: Gemini Flash
Winning joke: Cave Tour
...
Optional small header text above the panel: Winner: Gemini Flash - Cave Tour
Exact joke text to render in the right panel:
I took a guided tour through a cavern, and ninety minutes underground really breaks down a man’s spirit. At one point the guide hit a switch, plunged us into total pitch-black darkness, and whispered, 'Now, listen to the silence.' And all I could think was, if someone stabs this guy right now, there is simply no way any jury convicts the rest of us.
...
Round seed terms for exclusion reference only: Whodunit, Tour Guide, Waterfall, Forced attendance, Flirtatious, Haughty. These are not scene ingredients unless the winning joke explicitly uses them.
...
```sh
node scripts/prepare-feature-image-prompt.mjs --brief data/image-briefs/2026-09-15-external-real-2026-09-15.md
```

Read the generated `tmp/gemini-feature-images/2026-09-15-external-real-2026-09-15.prompt.txt` file directly when filling Gemini's composer.
...
```sh
node scripts/qa-feature-image.mjs --image tmp/gemini-feature-images/2026-09-15-external-real-2026-09-15.jpg --approved
```

After visual approval, render the episode with the approved preview image:

```sh
node scripts/run-tournament.mjs --episode-file data/inbox/external-real-2026-09-15.json --feature-image tmp/gemini-feature-images/2026-09-15-external-real-2026-09-15.jpg --feature-image-qa-approved
```
