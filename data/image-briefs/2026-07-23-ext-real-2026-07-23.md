...
Run: 2026-07-23-ext-real-2026-07-23
Winner: OpenAI GPT-5.4 Mini
Winning joke: Basic Package
...
- Show the literal comic beat of the joke: the cleaner has removed or scrubbed only one small stain while absurdly leaving the large chalk outline untouched, so the apartment still obviously reads as a crime scene.
- The cleaner must be a normal fully human adult with one clear head, face, neck, torso, two arms, and two legs. Do not hide, crop off, replace, merge, or obscure the head with a mop, hood blob, doorway, or cleaning gear.
- Make the cleaner look cheap and underqualified rather than scary: discount protective coveralls or janitorial clothes are fine, but the person should read as a bargain cleaner leaving too early, not as a killer, ghost, hazmat monster, or faceless mannequin.
- Show the cleaner walking out the apartment door or just exiting, with bucket, rag, or mop, while the host notices the untouched outline and the ridiculous half-finished result.
...
Optional small header text above the panel: Winner: OpenAI GPT-5.4 Mini - Basic Package
Exact joke text to render in the right panel:
I’m so stingy I hired the cheapest crime-scene cleaner online. He said the basic package included “one stain and no questions.” He cleaned around the chalk outline and left. Now my apartment doesn’t look like a crime scene—it looks like the victim had a very strict security deposit.
...
Round seed terms for exclusion reference only: Cyberpunk, Crime Scene Cleaner, Military Base, Having to stay behind, Spontaneous, Stingy. These are not scene ingredients unless the winning joke explicitly uses them.
...
```sh
node scripts/qa-feature-image.mjs --image tmp/gemini-feature-images/2026-07-23-ext-real-2026-07-23.jpg --approved
```

After visual approval, render the episode with the approved preview image:

```sh
node scripts/run-tournament.mjs --episode-file data/inbox/ext-real-2026-07-23.json --feature-image tmp/gemini-feature-images/2026-07-23-ext-real-2026-07-23.jpg --feature-image-qa-approved
```
