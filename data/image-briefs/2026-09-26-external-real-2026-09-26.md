...
Run: 2026-09-26-external-real-2026-09-26
Winner: Copilot
Winning joke: VIP Instructions
...
Optional small header text above the panel: Winner: Copilot - VIP Instructions
Exact joke text to render in the right panel:
I got a job as a personal assistant to a celebrity, which is basically a full-time experiment in blindly trusting someone richer than me. On my first day she texted, “Take my goat to a sound bath.” I thought it was a typo, but when your boss owns a goat and a sound bath, the typo is assuming you're the normal one.
...
Round seed terms for exclusion reference only: Cringe, Personal Assistant to a Celebrity, Concert, Having to blindly trust someone, Adventurous, Ignorant. These are not scene ingredients unless the winning joke explicitly uses them.
...
```sh
node scripts/prepare-feature-image-prompt.mjs --brief data/image-briefs/2026-09-26-external-real-2026-09-26.md
```

Read the generated `tmp/gemini-feature-images/2026-09-26-external-real-2026-09-26.prompt.txt` file directly when filling Gemini's composer.
...
```sh
node scripts/qa-feature-image.mjs --image tmp/gemini-feature-images/2026-09-26-external-real-2026-09-26.jpg --approved
```

After visual approval, render the episode with the approved preview image:

```sh
node scripts/run-tournament.mjs --episode-file data/inbox/external-real-2026-09-26.json --feature-image tmp/gemini-feature-images/2026-09-26-external-real-2026-09-26.jpg --feature-image-qa-approved
```
