...
Run: 2026-08-06-external-real-2026-08-06
Winner: Claude Sonnet 4.6
Winning joke: Bouncer Apocalypse
...
Optional small header text above the panel: Winner: Claude Sonnet 4.6 - Bouncer Apocalypse
Exact joke text to render in the right panel:
After the apocalypse I became a bouncer for the last nightclub on Earth. It's not hard. There's a list, there's two names on it, and one of them is a guy named Gary who's been dead since Tuesday. Still, I stand there all night, arms crossed, going 'not tonight' to zombies. They don't even want in. They just miss having somewhere to be turned away from.
...
Round seed terms for exclusion reference only: Post-Apocalyptic, Bouncer, Parade, Lost temper / composure, Gentle, Disorganized. These are not scene ingredients unless the winning joke explicitly uses them.
...
```sh
node scripts/qa-feature-image.mjs --image tmp/gemini-feature-images/2026-08-06-external-real-2026-08-06.jpg --approved
```

After visual approval, render the episode with the approved preview image:

```sh
node scripts/run-tournament.mjs --episode-file data/inbox/external-real-2026-08-06.json --feature-image tmp/gemini-feature-images/2026-08-06-external-real-2026-08-06.jpg --feature-image-qa-approved
```
